...
class Text {
	...
	*/
		...
		foreach ($Array as $Block) {
			...
			switch ($Block['Type']) {
				...
				case 'hide':
					...
					break;
				case 'mature':
					if (\Gazelle\G::$LoggedUser['EnableMatureContent']) {
						if (!empty($Block['Attr'])) {
						...
						$Str .= '<span class="mature_blocked" style="font-style: italic;"><a href="wiki.php?action=article&amp;id=1063">Mature content</a> has been blocked. You can choose to view mature content by editing your <a href="user.php?action=edit&amp;userid=' . \Gazelle\G::$LoggedUser['ID'] . '">settings</a>.</span>';
					}
					break;
				case 'img':
	...
	private static function smileys($Str) {
		if (!empty(\Gazelle\G::$LoggedUser['DisableSmileys'])) {
			return $Str;
		}
