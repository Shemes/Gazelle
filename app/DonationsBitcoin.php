...
class DonationsBitcoin
{
    ...
    public static function get_received()
    {
        if (defined('BITCOIN_RPC_URL')) {
            $Donations = \Gazelle\BitcoinRPC::listreceivedbyaddress();
        }
        if (empty($Donations)) {
            return [];
        }
    ...
    {
        if (defined('BITCOIN_RPC_URL')) {
            return \Gazelle\BitcoinRPC::getbalance();
        }
    }
    ...
    public static function get_address($UserID, $GenAddress = false)
    {
        ...
        \Gazelle\G::$DB->query("
			...
			FROM users_info
			WHERE UserID = '$UserID'");
        list($Addr) = \Gazelle\G::$DB->next_record();
        ...
        if (!empty($Addr)) {
            ...
            if (defined('BITCOIN_RPC_URL')) {
                $NewAddr = \Gazelle\BitcoinRPC::getnewaddress();
            }
            if (empty($NewAddr)) {
                error(0);
            }
    ...
    {
        if (defined('BITCOIN_RPC_URL')) {
            $Accounts = \Gazelle\BitcoinRPC::listreceivedbyaccount();
        }
        if (empty($Accounts)) {
            return 0.0;
        }
    ...
    public static function get_userids($Addresses)
    {
        ...
        if (\Gazelle\G::$DB->has_results()) {
            $UserIDs = \Gazelle\G::$DB->to_pair(0, 1);
        } else {
            $UserIDs = [];
        }
        \Gazelle\G::$DB->set_query_id($QueryID);
    ...
    {
        global $Debug;
        if (($OldAmount = \Gazelle\G::$Cache->get_value('btc_total_received')) === false) {
            $QueryID = \Gazelle\G::$DB->get_query_id();
            \Gazelle\G::$DB->query('
				SELECT IFNULL(SUM(Amount), 0)
				FROM donations_bitcoin');
            list($OldAmount) = \Gazelle\G::$DB->next_record(MYSQLI_NUM, false);
            \Gazelle\G::$DB->set_query_id($QueryID);
        }
        ...
        if ($NewAmount > $OldAmount) {
            ...
            \Gazelle\G::$Cache->cache_value('btc_total_received', $NewAmount, 0);
        }
    }
