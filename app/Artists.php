...
class Artists
{
    ...
    public static function get_artists($GroupIDs)
    {
        ...
        foreach ($GroupIDs as $GroupID) {
            ...
            $Artists = \Gazelle\G::$Cache->get_value('groups_artists_' . $GroupID);
            if (is_array($Artists)) {
        ...
        if (count($DBs) > 0) {
            ...
            \Gazelle\G::$DB->query("
				...
				ORDER BY ta.GroupID ASC,
					ta.Importance ASC,
					aa.Name ASC;");
            while (list($GroupID, $ArtistID, $ArtistName, $ArtistImportance, $AliasID) = \Gazelle\G::$DB->next_record(MYSQLI_BOTH, false)) {
            ...
            foreach ($DBs as $GroupID) {
                if (isset($New[$GroupID])) {
                    \Gazelle\G::$Cache->cache_value('groups_artists_' . $GroupID, $New[$GroupID]);
                } else {
                    \Gazelle\G::$Cache->cache_value('groups_artists_' . $GroupID, []);
                }
            }
    ...
    {
        $Results = \Gazelle\Artists::get_artists([$GroupID]);
        ...
        if (!empty($Artists)) {
            ...
            switch (count($Composers)) {
                case 0:
                    break;
                case 1:
                    $link .= \Gazelle\Artists::display_artist($Composers[0], $MakeLink, $Escape);
                    break;
                case 2:
                    $link .= \Gazelle\Artists::display_artist($Composers[0], $MakeLink, $Escape) . $ampersand . \Gazelle\Artists::display_artist($Composers[1], $MakeLink, $Escape);
                    break;
            }
            ...
            switch (count($MainArtists)) {
                case 0:
                    break;
                case 1:
                    $link .= \Gazelle\Artists::display_artist($MainArtists[0], $MakeLink, $Escape);
                    break;
                case 2:
                    $link .= \Gazelle\Artists::display_artist($MainArtists[0], $MakeLink, $Escape) . $ampersand . \Gazelle\Artists::display_artist($MainArtists[1], $MakeLink, $Escape);
                    break;
                default:
            ...
            /*if (!empty($Guests) &&  (count($MainArtists) + count($Composers) > 0) && (count($MainArtists) + count($Composers) + count($Conductors) < 3)) {
                switch (count($Guests)) {
                    case 1:
                        $link .= ' with '.\Gazelle\Artists::display_artist($Guests[0], $MakeLink, $Escape);
                        break;
                    case 2:
                        $link .= ' with '.\Gazelle\Artists::display_artist($Guests[0], $MakeLink, $Escape).$ampersand.\Gazelle\Artists::display_artist($Guests[1], $MakeLink, $Escape);
                        break;
                }
            }*/
            ...
            switch (count($Conductors)) {
                case 0:
                    break;
                case 1:
                    $link .= \Gazelle\Artists::display_artist($Conductors[0], $MakeLink, $Escape);
                    break;
                case 2:
                    $link .= \Gazelle\Artists::display_artist($Conductors[0], $MakeLink, $Escape) . $ampersand . \Gazelle\Artists::display_artist($Conductors[1], $MakeLink, $Escape);
                    break;
                default:
            ...
            switch (count($DJs)) {
                case 0:
                    break;
                case 1:
                    $link = \Gazelle\Artists::display_artist($DJs[0], $MakeLink, $Escape);
                    break;
                case 2:
                    $link = \Gazelle\Artists::display_artist($DJs[0], $MakeLink, $Escape) . $ampersand . \Gazelle\Artists::display_artist($DJs[1], $MakeLink, $Escape);
                    break;
                default:
                    $link = 'Various DJs';
            }
    ...
    {
        if ($MakeLink && !$Escape) {
            error('Invalid parameters to \Gazelle\Artists::display_artist()');
        } elseif ($MakeLink) {
    ...
    public static function delete_artist($ArtistID)
    {
        ...
        list($Name) = \Gazelle\G::$DB->next_record(MYSQLI_NUM, false);
        ...
        \Gazelle\G::$DB->query("
			...
			WHERE ArtistID = $ArtistID
				AND ArtistID != 0");
        $Requests = \Gazelle\G::$DB->to_array();
        foreach ($Requests as $Request) {
            list($RequestID) = $Request;
            \Gazelle\G::$DB->query('DELETE FROM requests WHERE ID=' . $RequestID);
            \Gazelle\G::$DB->query('DELETE FROM requests_votes WHERE RequestID=' . $RequestID);
            \Gazelle\G::$DB->query('DELETE FROM requests_tags WHERE RequestID=' . $RequestID);
            \Gazelle\G::$DB->query('DELETE FROM requests_artists WHERE RequestID=' . $RequestID);
        }

        // Delete artist
        \Gazelle\G::$DB->query('DELETE FROM artists_group WHERE ArtistID=' . $ArtistID);
        \Gazelle\G::$DB->query('DELETE FROM artists_alias WHERE ArtistID=' . $ArtistID);
        \Gazelle\G::$Cache->decrement('stats_artist_count');

        // Delete wiki revisions
        \Gazelle\G::$DB->query('DELETE FROM wiki_artists WHERE PageID=' . $ArtistID);

        // Delete tags
        \Gazelle\G::$DB->query('DELETE FROM artists_tags WHERE ArtistID=' . $ArtistID);

        // Delete artist comments, subscriptions and quote notifications
        \Gazelle\Comments::delete_page('artist', $ArtistID);

        \Gazelle\G::$Cache->delete_value('artist_' . $ArtistID);
        \Gazelle\G::$Cache->delete_value('artist_groups_' . $ArtistID);
        // Record in log

        if (!empty(\Gazelle\G::$LoggedUser['Username'])) {
            $Username = \Gazelle\G::$LoggedUser['Username'];
        } else {
            $Username = 'System';
        }
