...
class ArtistsSimilar extends Artist
{
    public $Artists = [];
    public $TotalScore = 0;

    public $xValues = [WIDTH => 1];
    public $yValues = [HEIGHT => 1];

    public $LargestDecimal = 0;
    public $LowestDecimal = 1;
    ...
    public function set_up()
    {
        ...
        while (list($ArtistID, $Name, $Score) = \Gazelle\G::$DB->next_record(MYSQLI_NUM, false)) {
            if ($Score < 0) {
                continue;
            }
            $this->Artists[$ArtistID] = new \Gazelle\Artist($ArtistID, $Name);
        ...
        // Build into array
        while (list($Artist1ID, $Artist2ID) = \Gazelle\G::$DB->next_record()) {
    ...
    // Pythagoras as fun!
    public function calculate_distance($SimilarityCoefficient, $Rootx, $Rooty)
    {
    ...
    public function similarity($Score, $TotalArtistScore)
    {
        return (pow(($Score / ($TotalArtistScore + 1)), (1 / 1)));
    }

    public function scan_array_range($Array, $Start, $Finish)
    {
        if ($Start < 0) {
            die($Start);
        }
