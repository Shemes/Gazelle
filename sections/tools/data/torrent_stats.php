...
if (!$TorrentStats = $Cache->get_value('new_torrent_stats')) {
	...
	$DB->query("SELECT COUNT(ID), SUM(Size), SUM(FileCount) FROM torrents WHERE Time > SUBDATE('".\Gazelle\Util\Time::sqltime()."', INTERVAL 1 DAY)");
	list($DayNum, $DaySize, $DayFiles) = $DB->next_record();
	$DB->query("SELECT COUNT(ID), SUM(Size), SUM(FileCount) FROM torrents WHERE Time > SUBDATE('".\Gazelle\Util\Time::sqltime()."', INTERVAL 7 DAY)");
	list($WeekNum, $WeekSize, $WeekFiles) = $DB->next_record();
	$DB->query("SELECT COUNT(ID), SUM(Size), SUM(FileCount) FROM torrents WHERE Time > SUBDATE('".\Gazelle\Util\Time::sqltime()."', INTERVAL 30 DAY)");
	...
	<div class="box">
		...
		<div class="pad">
			<ul class="stats nobullet">
				...
				<li><strong>Total size: </strong><?=Gazelle\Format::get_size($TotalSize)?></li>
				...
				<li><strong>Mean torrent size: </strong><?=Gazelle\Format::get_size($TotalSize / $TorrentCount)?></li>
				<li><strong>Mean files per torrent: </strong><?=number_format($TotalFiles / $TorrentCount)?></li>
				<li><strong>Mean filesize: </strong><?=Gazelle\Format::get_size($TotalSize / $TotalFiles)?></li>
			</ul>
		</div>
	</div>
	<br />
	<div class="box">
		...
		<div class="pad">
			<ul class="stats nobullet">
				...
				<li><strong>Size today: </strong><?=Gazelle\Format::get_size($DaySize)?></li>
				<li><strong>Files today: </strong><?=number_format($DayFiles)?></li>
				<br />
				<li><strong>Torrents this week: </strong><?=number_format($WeekNum)?></li>
				<li><strong>Size this week: </strong><?=Gazelle\Format::get_size($WeekSize)?></li>
				...
				<li><strong>Size per day this week: </strong><?=Gazelle\Format::get_size($WeekSize / 7)?></li>
				...
				<li><strong>Size this month: </strong><?=Gazelle\Format::get_size($MonthSize)?></li>
				...
				<li><strong>Size per day this month: </strong><?=Gazelle\Format::get_size($MonthSize / 30)?></li>
