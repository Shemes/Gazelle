...
class Votes {
	...
	public static function vote_link($GroupID, $Vote = '') {
		if (!\Gazelle\G::$LoggedUser['NoVoteLinks'] && check_perms('site_album_votes')) {
			$GroupVotes = self::get_group_votes($GroupID); ?>
			<span class="votespan brackets" style="white-space: nowrap;">
				Vote:
				<a href="#" onclick="UpVoteGroup(<?=$GroupID?>, '<?=\Gazelle\G::$LoggedUser['AuthKey']?>'); return false;" class="tooltip small_upvote vote_link_<?=$GroupID?><?=(!empty($Vote) ? ' hidden' : '')?>" style="font-weight: bolder;" title="Upvote">&and;</a>
				...
				<a href="#" onclick="DownVoteGroup(<?=$GroupID?>, '<?=\Gazelle\G::$LoggedUser['AuthKey']?>'); return false;" class="tooltip small_downvote vote_link_<?=$GroupID?><?=(!empty($Vote) ? ' hidden' : '')?>" style="font-weight: bolder;" title="Downvote">&or;</a>
				...
				<a href="#" onclick="UnvoteGroup(<?=$GroupID?>, '<?=\Gazelle\G::$LoggedUser['AuthKey']?>'); return false;" class="tooltip small_clearvote vote_clear_<?=$GroupID?><?=(empty($Vote) ? ' hidden' : '')?>" title="Clear your vote">x</a>
...
<?		}
	...
	public static function get_user_votes($UserID) {
		...
		$UserVotes = \Gazelle\G::$Cache->get_value("voted_albums_$UserID");
		if ($UserVotes === false) {
			...
			\Gazelle\G::$DB->query("
				...
				FROM users_votes
				WHERE UserID = $UserID");
			$UserVotes = \Gazelle\G::$DB->to_array('GroupID', MYSQLI_ASSOC, false);
			\Gazelle\G::$DB->set_query_id($QueryID);
			\Gazelle\G::$Cache->cache_value("voted_albums_$UserID", $UserVotes);
		}
		return $UserVotes;
	}
	...
	public static function get_group_votes($GroupID) {
		...
		$GroupVotes = \Gazelle\G::$Cache->get_value("votes_$GroupID");
		if ($GroupVotes === false) {
			$QueryID = \Gazelle\G::$DB->get_query_id();
			\Gazelle\G::$DB->query("
			...
			if (!\Gazelle\G::$DB->has_results()) {
				...
				$GroupVotes = \Gazelle\G::$DB->next_record(MYSQLI_ASSOC, false);
			}
			\Gazelle\G::$DB->set_query_id($QueryID);
			\Gazelle\G::$Cache->cache_value("votes_$GroupID", $GroupVotes, 259200); // 3 days
		}
		return $GroupVotes;
	}
	...
	public static function get_rank_all($GroupID) {
		...
		$Rankings = \Gazelle\G::$Cache->get_value('voting_ranks_overall');
		if ($Rankings === false) {
			$QueryID = \Gazelle\G::$DB->get_query_id();
			\Gazelle\G::$DB->query('
				...
				ORDER BY Score DESC
				LIMIT 100');
			$Rankings = self::calc_ranks(\Gazelle\G::$DB->to_pair(0, 1, false));
			\Gazelle\G::$DB->set_query_id($QueryID);
			\Gazelle\G::$Cache->cache_value('voting_ranks_overall', $Rankings, 259200); // 3 days
		}
	...
	public static function get_rank_year($GroupID, $Year) {
		...
		$Rankings = \Gazelle\G::$Cache->get_value("voting_ranks_year_$Year");
		if ($Rankings === false) {
			$QueryID = \Gazelle\G::$DB->get_query_id();
			\Gazelle\G::$DB->query("
				SELECT GroupID, Score
			...
			$Rankings = self::calc_ranks(\Gazelle\G::$DB->to_pair(0, 1, false));
			\Gazelle\G::$DB->set_query_id($QueryID);
			\Gazelle\G::$Cache->cache_value("voting_ranks_year_$Year", $Rankings, 259200); // 3 days
		}
	...
	public static function get_rank_decade($GroupID, $Year) {
		...
		$Rankings = \Gazelle\G::$Cache->get_value("voting_ranks_decade_$Year");
		if ($Rankings === false) {
			$QueryID = \Gazelle\G::$DB->get_query_id();
			\Gazelle\G::$DB->query("
				SELECT GroupID, Score
			...
			$Rankings = self::calc_ranks(\Gazelle\G::$DB->to_pair(0, 1, false));
			\Gazelle\G::$DB->set_query_id($QueryID);
			\Gazelle\G::$Cache->cache_value("voting_ranks_decade_$Year", $Rankings, 259200); // 3 days
		}
