...
if (isset($LoginCookie)) {
    ...
    \Gazelle\G::$LoggedUser = &$LoggedUser;
    ...
    $LoggedUser['Permissions']['MaxCollages'] += \Gazelle\Donations::get_personal_collages($LoggedUser['ID']);
...
function logout()
{
    ...
    if ($SessionID) {
        ...
        \Gazelle\G::$Cache->begin_transaction('users_sessions_' . \Gazelle\G::$LoggedUser['ID']);
        \Gazelle\G::$Cache->delete_row($SessionID);
        \Gazelle\G::$Cache->commit_transaction(0);
    }
    \Gazelle\G::$Cache->delete_value('user_info_' . \Gazelle\G::$LoggedUser['ID']);
    \Gazelle\G::$Cache->delete_value('user_stats_' . \Gazelle\G::$LoggedUser['ID']);
    \Gazelle\G::$Cache->delete_value('user_info_heavy_' . \Gazelle\G::$LoggedUser['ID']);
...
{
    global $SessionID;
    if (!$SessionID || !\Gazelle\G::$LoggedUser) {
...
{
    if (empty($_REQUEST['auth']) || $_REQUEST['auth'] != \Gazelle\G::$LoggedUser['AuthKey']) {
        send_irc('PRIVMSG ' . LAB_CHAN . ' :' . \Gazelle\G::$LoggedUser['Username'] . ' just failed authorize on ' . $_SERVER['REQUEST_URI'] . (!empty($_SERVER['HTTP_REFERER']) ? ' coming from ' . $_SERVER['HTTP_REFERER'] : ''));
...
function authorizeIfPost($Ajax = false)
{
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (empty($_POST['auth']) || $_POST['auth'] != \Gazelle\G::$LoggedUser['AuthKey']) {
            send_irc('PRIVMSG ' . LAB_CHAN . ' :' . \Gazelle\G::$LoggedUser['Username'] . ' just failed authorize on ' . $_SERVER['REQUEST_URI'] . (!empty($_SERVER['HTTP_REFERER']) ? ' coming from ' . $_SERVER['HTTP_REFERER'] : ''));
            error('Invalid authorization key. Go back, refresh, and try again.', $Ajax);
            return false;
        }
...
if (isset(\Gazelle\G::$LoggedUser['LockedAccount']) && !in_array($Document, $AllowedPages)) {
