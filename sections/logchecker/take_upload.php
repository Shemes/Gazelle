...
if ($TorrentID != 0 && $DB->has_results() && $FileCount > 0) {
    ...
    foreach ($_FILES['logfiles']['name'] as $Pos => $File) {
        ...
        $DB->query("INSERT INTO torrents_logs (TorrentID, Log, Details, Score, `Checksum`, `FileName`) VALUES ($TorrentID, '" . \Gazelle\Util\Db::string($LogText) . "', '" . \Gazelle\Util\Db::string($Details) . "', $Score, '" . Util\Db::enum_boolean($Checksum) . "', '" . \Gazelle\Util\Db::string($FileName) . "')");
    ...
    $DB->query("UPDATE torrents SET HasLogDB='1', LogScore={$LogScore}, LogChecksum='" . Util\Db::enum_boolean($LogChecksum) . "' WHERE ID='{$TorrentID}'");
