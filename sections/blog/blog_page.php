...
if (!isset($_GET['action']) || $_GET['action'] !== 'editblog') {
	...
	if (count($Blog) > 0 && \Gazelle\G::$LoggedUser['LastReadBlog'] < $Blog[0][0]) {
		$Cache->begin_transaction('user_info_heavy_'.\Gazelle\G::$LoggedUser['ID']);
		$Cache->update_row(false, array('LastReadBlog' => $Blog[0][0]));
		$Cache->commit_transaction(0);
		$DB->prepared_query("
		UPDATE users_info
		SET LastReadBlog = ?
		WHERE UserID = ?", $Blog[0][0], \Gazelle\G::$LoggedUser['ID']);
		...
		<div id="blog<?=$BlogID?>" class="box blog_post">
			<div class="head">
				...
				<?php	if (check_perms('admin_manage_blog')) { ?>
					...
					<a href="blog.php?action=deleteblog&amp;id=<?=$BlogID?>&amp;auth=<?=\Gazelle\G::$LoggedUser['AuthKey']?>" class="brackets">Delete</a>
				<?php	} ?>
			</div>
			<div class="pad">
				...
				<?php	if ($ThreadID) { ?>
					...
					<?php		if (check_perms('admin_manage_blog')) { ?>
						<a href="blog.php?action=deadthread&amp;id=<?=$BlogID?>&amp;auth=<?=\Gazelle\G::$LoggedUser['AuthKey']?>" class="brackets">Remove link</a>
						<?php
					}
				}
				?>
