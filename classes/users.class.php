...
class Users {
	...
	public static function get_classes() {
		global $Debug;
		// Get permissions
		list($Classes, $ClassLevels) = \Gazelle\G::$Cache->get_value('classes');
		if (!$Classes || !$ClassLevels) {
			...
			\Gazelle\G::$DB->query('
				...
				FROM permissions
				ORDER BY Level');
			$Classes = \Gazelle\G::$DB->to_array('ID');
			$ClassLevels = \Gazelle\G::$DB->to_array('Level');
			\Gazelle\G::$DB->set_query_id($QueryID);
			\Gazelle\G::$Cache->cache_value('classes', array($Classes, $ClassLevels), 0);
		}
	...
	public static function user_info($UserID) {
		global $Classes, $SSL;
		$UserInfo = \Gazelle\G::$Cache->get_value("user_info_$UserID");
		...
		if (empty($UserInfo) || empty($UserInfo['ID']) || !isset($UserInfo['Paranoia']) || empty($UserInfo['Class'])) {
			$OldQueryID = \Gazelle\G::$DB->get_query_id();
			...
			if (!\Gazelle\G::$DB->has_results()) { // Deleted user, maybe?
				$UserInfo = array(
						'ID' => $UserID,
						'Username' => '',
						'PermissionID' => 0,
				...
				$UserInfo = \Gazelle\G::$DB->next_record(MYSQLI_ASSOC, array('Paranoia', 'Title'));
			...
			\Gazelle\G::$Cache->cache_value("user_info_$UserID", $UserInfo, 2592000);
			\Gazelle\G::$DB->set_query_id($OldQueryID);
		}
	...
	public static function user_heavy_info($UserID) {
		...
		$HeavyInfo = \Gazelle\G::$Cache->get_value("user_info_heavy_$UserID");
		if (empty($HeavyInfo)) {
			...
			\Gazelle\G::$DB->query("
				...
				FROM users_main AS m
					INNER JOIN users_info AS i ON i.UserID = m.ID
				WHERE m.ID = '$UserID'");
			$HeavyInfo = \Gazelle\G::$DB->next_record(MYSQLI_ASSOC, array('CustomPermissions', 'SiteOptions'));
			...
			\Gazelle\G::$DB->query("
				...
				FROM users_levels
				WHERE UserID = $UserID");
			$PermIDs = \Gazelle\G::$DB->collect('PermissionID');
			...
			\Gazelle\G::$Cache->cache_value("user_info_heavy_$UserID", $HeavyInfo, 0);
		}
		return $HeavyInfo;
	}
	...
	public static function update_site_options($UserID, $NewOptions) {
		...
		\Gazelle\G::$DB->query("
			...
			FROM users_info
			WHERE UserID = $UserID");
		list($SiteOptions) = \Gazelle\G::$DB->next_record(MYSQLI_NUM, false);
		...
		// Update \Gazelle\G::$LoggedUser if the options are changed for the current
		if (\Gazelle\G::$LoggedUser['ID'] == $UserID) {
			\Gazelle\G::$LoggedUser = array_merge(\Gazelle\G::$LoggedUser, $NewOptions);
			\Gazelle\G::$LoggedUser['ID'] = $UserID; // We don't want to allow userid switching
		}
		return true;
	}
...
<?
	...
	public static function format_username($UserID, $Badges = false, $IsWarned = true, $IsEnabled = true, $Class = false, $Title = false, $IsDonorForum = false) {
		...
		$Str .= ($IsWarned && $UserInfo['Warned'] != '0000-00-00 00:00:00') ? '<a href="wiki.php?action=article&amp;id=114"'
					...
					. (\Gazelle\G::$LoggedUser['ID'] === $UserID ? ' - Expires ' . date('Y-m-d H:i', strtotime($UserInfo['Warned'])) : '')
					. '" class="tooltip" /></a>' : '';
	...
	public static function get_bookmarks($UserID) {
		...
		if (($Data = \Gazelle\G::$Cache->get_value("bookmarks_group_ids_$UserID"))) {
			...
			$BookmarkData = \Gazelle\G::$DB->to_array('GroupID', MYSQLI_ASSOC);
			\Gazelle\G::$DB->set_query_id($QueryID);
			\Gazelle\G::$Cache->cache_value("bookmarks_group_ids_$UserID",
	...
	public static function show_avatar($Avatar, $UserID, $Username, $Setting, $Size = 150, $ReturnHTML = True) {
		...
		switch ($Setting) {
			...
			case 2:
				...
				// Fallthrough
			case 3:
				switch (\Gazelle\G::$LoggedUser['Identicons']) {
					case 0:
						$Type = 'identicon';
						break;
					case 1:
	...
	public static function has_autocomplete_enabled($Type, $Output = true) {
		$Enabled = false;
		if (empty(\Gazelle\G::$LoggedUser['AutoComplete'])) {
			$Enabled = true;
		} elseif (\Gazelle\G::$LoggedUser['AutoComplete'] !== 1) {
			switch ($Type) {
				case 'search':
					if (\Gazelle\G::$LoggedUser['AutoComplete'] == 2) {
						$Enabled = true;
					}
					break;
				case 'other':
