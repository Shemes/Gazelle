...
sleep(5);
$Criteria = array();
$Criteria[] = array('From' => USER, 'To' => MEMBER,  'MinUpload' => 10 * 1024 * 1024 * 1024,  'MinRatio' => 0.7,  'MinUploads' => 0,  'MaxTime' => \Gazelle\Util\Time::timeMinus(3600 * 24 * 7));
$Criteria[] = array('From' => MEMBER, 'To' => POWER, 'MinUpload' => 25 * 1024 * 1024 * 1024,  'MinRatio' => 1.05, 'MinUploads' => 5,  'MaxTime' => \Gazelle\Util\Time::timeMinus(3600 * 24 * 7 * 2));
$Criteria[] = array('From' => POWER, 'To' => ELITE,  'MinUpload' => 100 * 1024 * 1024 * 1024, 'MinRatio' => 1.05, 'MinUploads' => 50, 'MaxTime' => \Gazelle\Util\Time::timeMinus(3600 * 24 * 7 * 4));
$Criteria[] = array('From' => ELITE, 'To' => TORRENT_MASTER, 'MinUpload' => 500 * 1024 * 1024 * 1024, 'MinRatio' => 1.05, 'MinUploads' => 500, 'MaxTime' => \Gazelle\Util\Time::timeMinus(3600 * 24 * 7 * 8));
$Criteria[] = array(
	'From' => TORRENT_MASTER,
	'To' => POWER_TM,
	...
	'MinRatio' => 1.05,
	'MinUploads' => 500,
	'MaxTime' => \Gazelle\Util\Time::timeMinus(3600 * 24 * 7 * 8),
	'Extra' => '
				(
...
foreach ($Criteria as $L) { // $L = Level
	...
	if (count($UserIDs) > 0) {
		...
		foreach ($UserIDs as $UserID) {
			...
			$DB->query("
					UPDATE users_info
					SET AdminComment = CONCAT('".\Gazelle\Util\Time::sqltime()." - Class changed to ".Users::make_class_string($L['To'])." by System\n\n', AdminComment)
					WHERE UserID = $UserID");
	...
	if (count($UserIDs) > 0) {
		...
		foreach ($UserIDs as $UserID) {
			...
			$DB->query("
					UPDATE users_info
					SET AdminComment = CONCAT('".\Gazelle\Util\Time::sqltime()." - Class changed to ".Users::make_class_string($L['From'])." by System\n\n', AdminComment)
					WHERE UserID = $UserID");
