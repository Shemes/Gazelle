...
class LastFM
{
    ...
    {
        $Username = \Gazelle\G::$Cache->get_value("lastfm_username_$UserID");
        if ($Username === false) {
            ...
            \Gazelle\G::$Cache->cache_value("lastfm_username_$UserID", $Username, 0);
        }
        return $Username;
    }
    ...
    {
        $ArtistEvents = \Gazelle\G::$Cache->get_value("artist_events_$ArtistID");
        if (empty($ArtistEvents)) {
            $ArtistEvents = self::lastfm_request('artist.getEvents', ['artist' => $Artist, 'limit' => $Limit]);
            \Gazelle\G::$Cache->cache_value("artist_events_$ArtistID", $ArtistEvents, 432000);
        }
        return $ArtistEvents;
    }
    ...
    {
        $Response = \Gazelle\G::$Cache->get_value("lastfm_user_info_$Username");
        if (empty($Response)) {
            $Response = self::lastfm_request('user.getInfo', ['user' => $Username]);
            \Gazelle\G::$Cache->cache_value("lastfm_user_info_$Username", $Response, 86400);
        }
        return $Response;
    }
    ...
    public static function compare_user_with($Username1, $Limit = 15)
    {
        ...
        if (\Gazelle\G::$DB->has_results()) {
            list($Username2) = \Gazelle\G::$DB->next_record();
            ...
            $Response = \Gazelle\G::$Cache->get_value("lastfm_compare_$Username1" . "_$Username2");
            if (empty($Response)) {
                ...
                \Gazelle\G::$Cache->cache_value("lastfm_compare_$Username1" . "_$Username2", $Response, 86400);
            }
        } else {
            $Response = null;
        }
    ...
    {
        $Response = \Gazelle\G::$Cache->get_value("lastfm_last_played_track_$Username");
        if (empty($Response)) {
            ...
            $Response = json_encode($Response);
            \Gazelle\G::$Cache->cache_value("lastfm_last_played_track_$Username", $Response, 7200);
        }
        return $Response;
    }
    ...
    {
        $Response = \Gazelle\G::$Cache->get_value("lastfm_top_artists_$Username");
        if (empty($Response)) {
            sleep(1);
            $Response = self::lastfm_request('user.getTopArtists', ['user' => $Username, 'limit' => $Limit]);
            $Response = json_encode($Response);
            \Gazelle\G::$Cache->cache_value("lastfm_top_artists_$Username", $Response, 86400);
        }
        return $Response;
    }
    ...
    {
        $Response = \Gazelle\G::$Cache->get_value("lastfm_top_albums_$Username");
        if (empty($Response)) {
            sleep(2);
            $Response = self::lastfm_request('user.getTopAlbums', ['user' => $Username, 'limit' => $Limit]);
            $Response = json_encode($Response);
            \Gazelle\G::$Cache->cache_value("lastfm_top_albums_$Username", $Response, 86400);
        }
        return $Response;
    }
    ...
    {
        $Response = \Gazelle\G::$Cache->get_value("lastfm_top_tracks_$Username");
        if (empty($Response)) {
            sleep(3);
            $Response = self::lastfm_request('user.getTopTracks', ['user' => $Username, 'limit' => $Limit]);
            $Response = json_encode($Response);
            \Gazelle\G::$Cache->cache_value("lastfm_top_tracks_$Username", $Response, 86400);
        }
        return $Response;
    }
    ...
    {
        $Response = \Gazelle\G::$Cache->get_value("lastfm_artist_chart_$Username");
        if (empty($Response)) {
            $Response = self::lastfm_request('user.getWeeklyArtistChart', ['user' => $Username]);
            $Response = json_encode($Response);
            \Gazelle\G::$Cache->cache_value("lastfm_artist_chart_$Username", $Response, 86400);
        }
        return $Response;
    }
    ...
    {
        $Response = \Gazelle\G::$Cache->get_value("lastfm_top_artists_$Limit");
        if (empty($Response)) {
            $Response = self::lastfm_request('chart.getTopArtists', ['limit' => $Limit]);
            $Response = json_encode($Response);
            \Gazelle\G::$Cache->cache_value("lastfm_top_artists_$Limit", $Response, 86400);
        }
        return $Response;
    }
    ...
    {
        $Response = \Gazelle\G::$Cache->get_value("lastfm_hyped_artists_$Limit");
        if (empty($Response)) {
            $Response = self::lastfm_request('chart.getHypedArtists', ['limit' => $Limit]);
            $Response = json_encode($Response);
            \Gazelle\G::$Cache->cache_value("lastfm_hyped_artists_$Limit", $Response, 86400);
        }
        return $Response;
    }
    ...
    public static function clear_cache($Username, $UserID)
    {
        ...
        if (empty($Response)) {
            ...
            \Gazelle\G::$Cache->cache_value("lastfm_clear_cache_$UserID", 1, 600);
            \Gazelle\G::$Cache->delete_value("lastfm_user_info_$Username");
            \Gazelle\G::$Cache->delete_value("lastfm_last_played_track_$Username");
            \Gazelle\G::$Cache->delete_value("lastfm_top_artists_$Username");
            \Gazelle\G::$Cache->delete_value("lastfm_top_albums_$Username");
            \Gazelle\G::$Cache->delete_value("lastfm_top_tracks_$Username");
            $QueryID = \Gazelle\G::$DB->get_query_id();
            \Gazelle\G::$DB->query('
				SELECT username
				FROM lastfm_users
            ...
            if (\Gazelle\G::$DB->has_results()) {
                ...
                \Gazelle\G::$Cache->delete_value("lastfm_compare_{$Username}_$Username2");
            }
            \Gazelle\G::$DB->set_query_id($QueryID);
        }
    }
    ...
    private static function lastfm_request($Method, $Args)
    {
        ...
        $RecentFails = (int)\Gazelle\G::$Cache->get_value($RecentFailsKey);
        if ($RecentFails > 5) {
        ...
        if (is_array($Args)) {
            ...
            curl_close($Curl);
            if ($Errno) {
                \Gazelle\G::$Cache->cache_value($RecentFailsKey, $RecentFails + 1, 1800);
                return false;
            }
