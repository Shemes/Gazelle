...
class Misc {
	...
	public static function send_pm($ToID, $FromID, $Subject, $Body, $ConvID = '') {
		...
		if ($ConvID == '') {
			...
			\Gazelle\G::$DB->query("
				...
				VALUES ('$Subject')");
			$ConvID = \Gazelle\G::$DB->inserted_id();
			\Gazelle\G::$DB->query("
		...
		foreach ($ToID as $ID) {
			\Gazelle\G::$DB->query("
				...
				WHERE UnRead = '1'
					AND UserID = '$ID'
					AND InInbox = '1'");
			list($UnRead) = \Gazelle\G::$DB->next_record();
			\Gazelle\G::$Cache->cache_value("inbox_new_$ID", $UnRead);
		}
		...
		\Gazelle\G::$DB->query("
			...
			FROM users_main
			WHERE ID = '$FromID'");
		list($SenderName) = \Gazelle\G::$DB->next_record();
		foreach ($ToID as $ID) {
			\Gazelle\G::$DB->query("
				SELECT COUNT(ConvID)
				FROM pm_conversations_users
	...
	public static function create_thread($ForumID, $AuthorID, $Title, $PostBody) {
		...
		list($AuthorName) = \Gazelle\G::$DB->next_record();
		...
		$TopicID = \Gazelle\G::$DB->inserted_id();
		$Posts = 1;
		...
		list($Forum,,, $Stickies) = \Gazelle\G::$Cache->get_value("forums_$ForumID");
		if (!empty($Forum)) {
			...
			\Gazelle\G::$DB->query("
				...
				FROM forums_topics
				WHERE ID ='$TopicID'");
			list($IsLocked, $IsSticky, $NumPosts) = \Gazelle\G::$DB->next_record();
			...
			\Gazelle\G::$Cache->cache_value("forums_$ForumID", array($Forum, '', 0, $Stickies), 0);
		}

		//Update the forum root
		\Gazelle\G::$Cache->begin_transaction('forums_list');
		$UpdateArray = array(
			'NumPosts' => '+1',
			'NumTopics' => '+1',
			'LastPostID' => $PostID,
		...
		\Gazelle\G::$Cache->update_row($ForumID, $UpdateArray);
		...
		\Gazelle\G::$Cache->begin_transaction('thread_'.$TopicID.'_catalogue_'.$CatalogueID);
		$Post = array(
			'ID' => $PostID,
			'AuthorID' => \Gazelle\G::$LoggedUser['ID'],
		...
		\Gazelle\G::$Cache->insert('', $Post);
		\Gazelle\G::$Cache->commit_transaction(0);

		\Gazelle\G::$Cache->begin_transaction('thread_'.$TopicID.'_info');
		\Gazelle\G::$Cache->update_row(false, array('Posts' => '+1', 'LastPostAuthorID' => $AuthorID));
		\Gazelle\G::$Cache->commit_transaction(0);
	...
	public static function get_tags($TagNames) {
		...
		foreach ($TagNames as $Index => $TagName) {
			$Tag = \Gazelle\G::$Cache->get_value("tag_id_$TagName");
			if (is_array($Tag)) {
		...
		if (count($TagNames) > 0) {
			...
			$SQLTagIDs = \Gazelle\G::$DB->to_array();
			\Gazelle\G::$DB->set_query_id($QueryID);
			foreach ($SQLTagIDs as $Tag) {
				$TagIDs[$Tag['ID']] = $Tag['Name'];
				\Gazelle\G::$Cache->cache_value('tag_id_'.$Tag['Name'], $Tag, 0);
			}
		}
	...
	public static function get_alias_tag($BadTag) {
		...
		\Gazelle\G::$DB->query("
			...
			LIMIT 1");
		if (\Gazelle\G::$DB->has_results()) {
			list($AliasTag) = \Gazelle\G::$DB->next_record();
		} else {
			$AliasTag = $BadTag;
		}
		\Gazelle\G::$DB->set_query_id($QueryID);
