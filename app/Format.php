...
class Format
{
     ...
     * @param string $UserIDKey Key in _REQUEST for a user ID parameter, which if given will be compared to \Gazelle\G::$LoggedUser[ID]
     *
    ...
    {
        if ($UserIDKey && isset($_REQUEST[$UserIDKey]) && \Gazelle\G::$LoggedUser['ID'] != $_REQUEST[$UserIDKey]) {
            return '';
        }
        $Pass = true;
        if (!is_array($Tests)) {
