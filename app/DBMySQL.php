...
* Escaping a string
    ...
    \Gazelle\Util\Db::string($str);
    Is a wrapper for $DB->escape_str(), which is a wrapper for
    mysqli_real_escape_string(). The \Gazelle\Util\Db::string() function exists so that you
...
class DBMySQL
{
     ...
     * as via \Gazelle\Util\Db::string) is still not as safe for using prepared statements so for queries
    ...
    // You should use \Gazelle\Util\Db::string() instead.
