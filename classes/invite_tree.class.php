...
<?
		...
		list($TreePosition, $TreeID, $TreeLevel) = \Gazelle\G::$DB->next_record(MYSQLI_NUM, false);
		...
		\Gazelle\G::$DB->query("
			...
			LIMIT 1");
		if (\Gazelle\G::$DB->has_results()) {
			list($MaxPosition) = \Gazelle\G::$DB->next_record(MYSQLI_NUM, false);
		} else {
			$MaxPosition = false;
		}
		$TreeQuery = \Gazelle\G::$DB->query("
			SELECT
				it.UserID,
				Enabled,
				PermissionID,
		...
		ob_start();
		while (list($ID, $Enabled, $Class, $Donor, $Uploaded, $Downloaded, $Paranoia, $TreePosition, $TreeLevel) = \Gazelle\G::$DB->next_record(MYSQLI_NUM, false)) {
...
<?
			if (\Gazelle\Paranoia::check(array('uploaded', 'downloaded'), $Paranoia, $UserClass)) {
...
<?
			$PreviousTreeLevel = $TreeLevel;
			\Gazelle\G::$DB->set_query_id($TreeQuery);
		}
