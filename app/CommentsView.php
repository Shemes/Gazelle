...
class CommentsView
{
     ...
     * @param array $Thread An array as returned by \Gazelle\Comments::load
     ...
     */
    public function render_comment($AuthorID, $PostID, $Body, $AddedTime, $EditedUserID, $EditedTime, $Link, $Unread = false, $Header = '', $Tools = true)
    {
        $UserInfo = \Users::user_info($AuthorID);
        $Header = '<strong>' . \Users::format_username($AuthorID, true, true, true, true, false) . '</strong> ' . \Gazelle\Util\Time::timeDiff($AddedTime) . $Header; ?>
...
<?php			if ($AuthorID == \Gazelle\G::$LoggedUser['ID'] || check_perms('site_moderate_forums')) {
                ?>
...
<?php
            if (check_perms('users_warn') && $AuthorID != \Gazelle\G::$LoggedUser['ID'] && \Gazelle\G::$LoggedUser['Class'] >= $UserInfo['Class']) {
                ?>
...
<?php		if (\Users::has_avatars_enabled()) {
            ?>
				...
				<?=\Users::show_avatar($UserInfo['Avatar'], $AuthorID, $UserInfo['Username'], \Gazelle\G::$LoggedUser['DisableAvatars'])?>
				</td>
<?php
        } ?>
